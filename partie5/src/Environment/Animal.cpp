...
Animal::Animal(ToricPosition pos){
    setPosition(pos);
}
...
{
    auto const animal = buildSprite(getPosition().toVec2d(), 50.0, getAppTexture(getTexture()), getDirection()/DEG_TO_RAD); //getSize?
    target.draw(animal);
    if (isDebugOn()) {
        Vec2d li;
...
Animal::Animal(double pointvie1, int esperancevie1, Vec2d position1)
{
    ...
    direction= 0;//uniform(0.0, TAU);
}


Animal::Animal(double pointvie1, Vec2d position1) //???
    :Animal(pointvie1, getAppConfig().animal_default_lifespan, position1)

{direction=0;}

Animal::Animal(Vec2d pos, double pointvie1, int esperancevie1)
{
    setPosition(pos);
    pointvie=pointvie1;
    esperancevie=esperancevie1;
...
Animal::Animal(ToricPosition pos, double pointvie1, int esperancevie1){
   setPosition(pos);
    pointvie=pointvie1;
    esperancevie=esperancevie1;
}
...
void Animal:: move(sf::Time dt)
{
    ...
    temps = temps + dt;
    Vec2d vecteur_vitesse = this->getSpeed() * dir.fromAngle(direction);
    ...
    vecteur_vitesse = this->getSpeed() * dir.fromAngle(direction);
...
void Animal:: setpointdevie(double point){
   pointvie=point;
}

double Animal:: getPointVie() const{
    return pointvie;
}
